...
bool HippoLog::InitLogFile(/*const char* fn*/)
{
	...
	m_pFile->open(m_logName.c_str(),std::ios::app);

	return true;
}

std::string HippoLog::GetLogName()
{
	if (m_logName.empty())
		m_logName=HippoLog::GenerateFileName("EngineLog/functional_test");

	return m_logName;
}

void HippoLog::SetLogName(const char* logname)
{
...
std::string HippoLog::GenerateFileName(const char* baseName)
{
	...
	char buff[256];
	sprintf_s(buff,256,"%s_%lu_%s.log",baseName,pid,szParsedString);
	...
	:m_content(str)
{
	...
	Hippo_WriteConsole(CC_WHITE,"%s Begin",m_content.c_str());
}
HippoFunLogObj::~HippoFunLogObj()
{
	//Hippo_WriteConsoleAndLog(CC_WHITE,"%s End",m_content.c_str());
	Hippo_WriteConsole(CC_WHITE,"%s End",m_content.c_str());
}