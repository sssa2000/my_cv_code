...
//自定义的消息处理函数
typedef boostl_tr1::function<LRESULT (UINT uMsg, WPARAM wParam, LPARAM lParam)> CustomMsgProc;
//on size事件
typedef boostl_tr1::function<int (HippoSizeEvent& e)> WndSizeCallback;
//keydown的回调函数，参数是窗口和按下的键的键码
typedef boostl_tr1::function<int (unsigned int)> KeyCallback;
//mouse key down的回调函数
typedef boostl_tr1::function<int (HippoMouseEvent& e)> MouseKeyCallback;
//mouse wheel
typedef boostl_tr1::function<int (HippoWheelEvent& w)> MouseWheelCallback;

//file drop
typedef boostl_tr1::function<void (HDROP hdrop)> DropFileCallback;
