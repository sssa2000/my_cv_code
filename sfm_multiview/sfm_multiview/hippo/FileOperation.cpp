...
DWORD _GetFileSize(const char* file_full_name)
{
	if(file_full_name)
	{
		...
		HANDLE hFile = CreateFileA(file_full_name,// file to open
	...
	if(from_dir )
	{
		...
		for(int i=0;i<totalsize;++i)
		{
			...
			if(!res)
			{
				...
				char errorcode[128];
				sprintf(errorcode,"   Error Code: %lu",info);
...
bool SaveHistoryCase(const char * filename, std::vector<std::string>& historyFiles)
{
	if (!filename)
		return false;
	std::ofstream ofile(filename);

	for (auto& name : historyFiles)
	{
		ofile << name<<std::endl;
	}
	ofile.close();
...
bool ReadHistoryCase(const char * filename, std::vector<std::string>& historyFiles)
{
	if (!filename)
		return false;
	...
	std::string tmp;
	while (getline(ifile, tmp)) // line中不包括每行的换行符  
	{
		historyFiles.push_back(tmp);
	}
	ifile.close();
	return true;
}
